...
function getLobyList(mu_type, target) {
	...
	$.ajax({
			...
			$('#btnMoneyAct').off().on('click', function() {
				...
				$.ajax({
					...
					error : function(request, status, error) {
						if (status != "" && error != "") {
							alert('btnMoneyAct 처리중 오류가 발생했습니다.\n\n문제가 지속될 경우 관리자에게 문의해주세요.');
						}
						location.reload()
					}
				});
					...
					$.ajax({
						...
						success : function(data) {
							...
							$.ajax({
								...
								error : function(request, status, error) {
									if (status != "" && error != "") {
										alert('.companys-title, .pragmatic_event 처리중 오류가 발생했습니다.\n\n문제가 지속될 경우 관리자에게 문의해주세요.');
									}
									statusment = false;
								}
							});
						...
						error : function(request, status, error) {
							if (status != "" && error != "") {
								alert('.companys-title, .pragmatic_event 전체 처리중 오류가 발생했습니다.\n\n문제가 지속될 경우 관리자에게 문의해주세요.');
							}
							statusment = false;
						}
					});
		...
		error : function(request, status, error) {
			if (status != "" && error != "") {
				alert('.companys-title, .pragmatic_event 라스트 처리중 오류가 발생했습니다.\n\n문제가 지속될 경우 관리자에게 문의해주세요.');
			}
			statusment = false;
		}
	});
	...
	$.ajax({
		...
		success : function(data) {
			...
			$.ajax({
				...
				error : function(request, status, error) {
					if (status != "" && error != "") {
						alert('getSlotView 처리중 오류가 발생했습니다.\n\n문제가 지속될 경우 관리자에게 문의해주세요.');
					}
					statusment = false;
				}
			});
		...
		error : function(request, status, error) {
			if (status != "" && error != "") {
				alert('getSlotView2 처리중 오류가 발생했습니다.\n\n문제가 지속될 경우 관리자에게 문의해주세요.');
			}
			statusment = false;
		}
	});
